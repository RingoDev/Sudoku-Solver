...
function getTransformationMatrix(corners, maxLength) {
    ...
    const dst = cv.matFromArray(4, 1, cv.CV_32FC2, [
        0, 0, maxLength - 1, 0,
        0, maxLength - 1, maxLength - 1, maxLength - 1]);
...
function getCornerPoints(pointArray, originalWidth, originalHeight) {
    const CONSTANT_CORNERS = [{x: 0, y: 0}, {x: originalWidth, y: 0}, {x: originalWidth, y: originalHeight}, {
        x: 0,
        y: originalHeight
    }]
...
function findLargestBlob(mat, originalWidth, originalHeight) {
    ...
    for (let i = 0; i * 5 < statsList.length; i++) {
        ...
        if (width !== originalWidth && height !== originalHeight && area > maxArea) {
            maxArea = area;
            boxStats.label = i;
...
function combineDigits(digits) {
    ...
    for (let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
            digits[i * 9 + j].copyTo(result.roi(new cv.Rect(i * 28, j * 28, digits[i * 9 + j].cols, digits[i * 9 + j].rows)));
            digits[i * 9 + j].delete()
        }
    }
    return result;
...
function extractDigit(undistorted, square) {
    const digit = undistorted.roi(new cv.Rect(square.p1.x, square.p1.y, square.p2.x - square.p1.x, square.p2.y - square.p1.y));
    ...
    if (maxArea === -1) {
        ...
        boundingBox = digit.roi(new cv.Rect(boxStats.left_x, boxStats.top_y, boxStats.width, boxStats.height));
...
function pad_to_square(img) {
    ...
    let h = img.rows;
    let w = img.cols;
    let t_pad = 0, b_pad = 0, l_pad = 0, r_pad = 0
    ...
    cv.copyMakeBorder(img, img, t_pad, b_pad, l_pad, r_pad, cv.BORDER_CONSTANT, new cv.Scalar(0))
    console.assert(Math.abs(img.rows - img.cols) <= 1, 'Digit is not square enough')
}
