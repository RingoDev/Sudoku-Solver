...
export default function hiddenPairs(input: sudoku): [boolean, sudoku] {
    ...
    for (let rowIndex = 0; rowIndex < sudoku.length; rowIndex++) {
        for (let colIndex = 0; colIndex < sudoku[rowIndex].length; colIndex++) {
            ...
            if (found !== undefined) {
                console.debug("Found a hidden Pair in row", [rowIndex, colIndex], found)
                if (removeFromFields(sudoku, found, [rowIndex, colIndex])) changed = true
            }

            found = hiddenPairColumn(sudoku, [rowIndex, colIndex])
            if (found !== undefined) {
                console.debug("Found a hidden Pair in column ", [rowIndex, colIndex], found)
                if (removeFromFields(sudoku, found, [rowIndex, colIndex])) changed = true
            }
            ...
            if (found !== undefined) {
                ...
                console.debug("Found a hidden Pair in 3x3 ", [rowIndex, colIndex], found)
                if (removeFromFields(sudoku, found, [rowIndex, colIndex])) changed = true
            }
        }
    }
...
function removeNonPair(sudoku: sudoku, field: digit[], pair: [number, number], pos: [rowIndex: number, colIndex: number]) {
    let changed = false;
 ...
 */
function hiddenPairRow(sudoku: sudoku, pos: position): [position, [number, number]] | undefined {
    ...
    for (let j = 0; j < sudoku.length; j++) {
        ...
        if (overlap.length === 2) return [[row, j], [overlap[0], overlap[1]]];
    }
}
 ...
 * @return the position of the counterpart of the hidden pair and the values of the pair
 */
function hiddenPairColumn(sudoku: sudoku, pos: position): [position, [number, number]] | undefined {
    const [row, column] = pos;
    ...
    for (let i = 0; i < sudoku.length; i++) {
        ...
        if (overlap.length === 2) return [[i, column], [overlap[0], overlap[1]]];
    }
}
 ...
 * @return the position of the counterpart of the hidden pair and the values of the pair
 */
function hiddenPair3x3(sudoku: sudoku, pos: position): [position, [number, number]] | undefined {
    const [row, column] = pos;
    ...
    for (let r = 0; r < 3; r++) {
        for (let c = 0; c < 3; c++) {
            ...
            for (let x = 0; x < 3; x++) {
                for (let y = 0; y < 3; y++) {
                    if ((boxRow + x === row && boxCol + y === column) || (x === r && y === c)) continue; // dont check pair against first and current
            ...
            if (overlap.length === 2) return [[boxRow + r, boxCol + c], [overlap[0], overlap[1]]];
        }
    }
}

function removeFromFields(sudoku: sudoku, found: [[row: number, column: number], [number, number]], pos: [rowIndex: number, colIndex: number]) {
