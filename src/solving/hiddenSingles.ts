...
export default function hiddenSingles(input: sudoku): [boolean, sudoku] {
...
function countInColumn(sudoku: sudoku, column: number, value: digit) {
    let sum = 0;
    for (let i = 0; i < sudoku.length; i++) {
...
// returns the amount of fields a possible number appears in a column
function countInRow(sudoku: sudoku, row: number, value: digit) {
    let sum = 0;
    for (let i = 0; i < sudoku.length; i++) {
...
function countInField(sudoku: sudoku, row: number, column: number, value: digit) {
    let sum = 0;
