...
export default function hiddenTriples(input: sudoku): [boolean, sudoku] {
    ...
    for (let rowIndex = 0; rowIndex < sudoku.length; rowIndex++) {
        for (let colIndex = 0; colIndex < sudoku[rowIndex].length; colIndex++) {
            ...
            if (found !== undefined) {
                console.debug("Found a hidden Triple in row", [rowIndex, colIndex], found)
                if (removeFromFields(sudoku, [rowIndex, colIndex], found[0], found[1], found[2])) changed = true
            }

            found = hiddenTripleColumn(sudoku, [rowIndex, colIndex])
            if (found !== undefined) {
                console.debug("Found a hidden Triple in column ", [rowIndex, colIndex], found)
                if (removeFromFields(sudoku, [rowIndex, colIndex], found[0], found[1], found[2])) changed = true
            }
            ...
            if (found !== undefined) {
                ...
                console.debug("Found a hidden Triple in 3x3 ", [rowIndex, colIndex], found)
                if (removeFromFields(sudoku, [rowIndex, colIndex], found[0], found[1], found[2])) changed = true
            }
        }
    }
...
function removeNonTriple(sudoku: sudoku, field: digit[], triple: [number, number, number], pos: position) {
    let changed = false;
...
function reduceOverlapRow(sudoku: sudoku, pos: [row: number, column: number], j: number, k: number, overlap: digit[]) {
...
function reduceOverlapColumn(sudoku: sudoku, pos: [row: number, column: number], i: number, k: number, overlap: digit[]) {
    const [row, column] = pos
 ...
 */
function hiddenTripleRow(sudoku: sudoku, pos: position): [position, position, [number, number, number]] | undefined {
    const [row, column] = pos;
    ...
    for (let j = 0; j < sudoku.length; j++) {
        ...
        for (let k = j + 1; k < sudoku.length; k++) {
            ...
            if (overlap.length === 3) return [[row, j], [row, k], [overlap[0], overlap[1], overlap[2]]];
        }
    }
}
 ...
 */
function hiddenTripleColumn(sudoku: sudoku, pos: position): [position, position, [number, number, number]] | undefined {
    const [row, column] = pos;
    ...
    for (let i = 0; i < sudoku.length; i++) {
        ...
        for (let k = i + 1; k < sudoku.length; k++) {
            ...
            // still 3 overlap, this is actually a hidden triple
            if (overlap.length === 3) return [[row, i], [row, k], [overlap[0], overlap[1], overlap[2]]];
        }
    }
}
 ...
 * @return the position of the counterpart of the hidden pair and the values of the pair
 */
function hiddenTriple3x3(sudoku: sudoku, [row, column]: position): [position, position, [number, number, number]] | undefined {
    ...
    for (let r = 0; r < 3; r++) {
        for (let c = 0; c < 3; c++) {
            ...
            for (let x = 0; x < 3; x++) {
                for (let y = 0; y < 3; y++) {
                    if ((boxRow + x === row && boxCol + y === column) || (x === r && y === c)) continue; // dont check against first and second
                    ...
                    overlap = reduceOverlap3x3(sudoku, [row, column], [r, c], [x, y], overlap);

                    // still 3 overlap, this is actually a hidden triple
                    if (overlap.length === 3) return [[boxRow + r, boxCol + c], [boxRow + x, boxCol + y], [overlap[0], overlap[1], overlap[2]]];
                }
            }
        }
    }
...
function reduceOverlap3x3(sudoku: sudoku, [row, column]: position, [bRowSecond, bColSecond]: position, [bRowThird, bColThird]: position, overlap: digit[]) {
    ...
    for (let x = 0; x < 3; x++) {
        for (let y = 0; y < 3; y++) {
            if ((boxRow + x === row && boxCol + y === column)
                || (x === bRowSecond && y === bColSecond)
                || (x === bRowThird && y === bColThird)) continue; // dont check against first, second and third
...
function removeFromFields(sudoku: sudoku, first: position, second: position, third: position, triple: [number, number, number]) {
    ...
    if (typeof field1 === "number" || typeof field2 === "number" || typeof field3 === "number") return false; // should never happen
 ...
 * @param opposite
 */
function filter(array: digit[], values: number[], opposite: boolean = false): [digit[], boolean] {
