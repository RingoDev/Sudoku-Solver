...
export default function StaticImage() {
    ...
    async function onClick() {
        ...
        if (canvasEl !== null && canvasEl.current !== null) {
            ...
            if (ctx !== null && imgElement.current !== null) {
                ...
                const image = ctx.getImageData(0, 0,imgElement.current.width,imgElement.current.height)
                // Load the model
                await cv.load()
                // Processing image
                const processedImage = await cv.imageProcessing(image) as { data: { payload: ImageData } }
    ...
    return (
        <>
            <img src={'img/test.jpg'} ref={imgElement}/>
            <div
                style={{
                    display: 'flex',
